...
library(foreign);library(car);library(R2WinBUGS);library(MASS);library(poLCA)
library(ltm)
...
mus$ses <- NA
      for (i in 1:ncol(sesmat)) {
            ...
            for (j in 1:length(seltab)) {
                  mus$ses[(mus$country==cnames[i]) & (sesmat[,i]==
                  names(seltab)[j])] <- j-1
            }
            mus$ses[grep("Refused",sesmat[,i])] <- NA
            mus$ses[grep("know",sesmat[,i])] <- NA
            mus$ses[mus$country==cnames[i]] <- mus$ses[mus$country==cnames[i]]/
                  max(mus$ses[mus$country==cnames[i]],na.rm=T)
}
...
mus$ed3 <- 0+(mus$educ==3)
      ...
      ##------------------------------------------------------------------------
      ## Percent of country that is Muslim. Source: Pew study, 
      #           http://pewforum.org/docs/?DocID=450
      ## Distance in 1000 miles between country capital and Jerusalem. 
      #           Source: Google maps
      ## Imports from the US, in thousands of dollars, 2007. 
      ...
      ## Total Economic Assistance (no military), 2007, in millions, 
      #           historical 2007 $US
      ## Source: U.S. Overseas Loans and Grants [Greenbook] 
      #           http://gbk.eads.usaidallnet.gov/
      ## Gallup world poll: http://www.gallup.com/poll/142631/Worldwide-
      #           Leadership-Approval.aspx
      ##------------------------------------------------------------------------
...
save(cnames, file="Data/cnames.Rda");save(cvars, file="Data/cvars.Rda")
save(dat, file="Data/dta.Rda");save(edmat, file="Data/edmat.Rda")
save(gap07, file="Data/gap07.Rda");save(mus, file="Data/mus.Rda")
